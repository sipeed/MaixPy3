...
static int Model_init(ModelObject *self, PyObject *args, PyObject *kwds)
{
    ...
    if(strcmp(model_type, "awnn") == 0)
    {
        ...
        if(!o_inputs || !PyList_Check(o_inputs))
        {
            PyErr_SetString(PyExc_ValueError, "arg opt need inputs key, value is list");
            return -1;
        }
        Py_ssize_t inputs_len = PyList_Size(o_inputs);
        PyObject *o_outputs = PyDict_GetItemString(o_opt, "inputs");
        if(!o_outputs || !PyList_Check(o_outputs))
        {
            PyErr_SetString(PyExc_ValueError, "arg opt need outputs key, value is list");
            return -1;
        }
        Py_ssize_t outputs_len = PyList_Size(o_outputs);
        ...
        {
            inputs_names[i] = (char*)PyUnicode_DATA(PyList_GetItem(o_inputs, i));
        }
        for(Py_ssize_t i=0; i<outputs_len; ++i)
        {
            outputs_names[i] = (char*)PyUnicode_DATA(PyList_GetItem(o_outputs, i));
        }
...
static PyObject* Model_forward(ModelObject *self, PyObject *args, PyObject *kw_args)
{
    ...
    if(o_inputs->ob_type->tp_name);
    // else if()
    // {

    // }
    return PyUnicode_InternFromString("echo from forward");
}
